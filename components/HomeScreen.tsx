...
export default function Home(){
  ...
  const { message, startScan, humidity, temperature, scanning, pressure, gasResistance } = useBluetooth();
    ...
    return(<>
        <View className='items-center justify-center h-[220px]'>
          ...
          <View className='flex h-auto'>
            <View className=' h-auto p-[5px] bg-white rounded-[20px]'>
              ...
              <View className='flex items-center justify-between rounded-[10px]'>
        ...
        { tab == `Gas` && 
          <SensorModal logo={ GasLogo } Title={`Gas`} sensorModel={`BME680`} currentReading={`VOC Index: ${ gasResistance }`} setTab={ setTab }/>
        }
        { tab == `Humidity` && 
          <SensorModal Title={`Humidity`} sensorModel={`SHT31`} currentReading={`Humidity: ${ humidity }`} setTab={ setTab } logo={ HumidityLogo }/>
        }
        { tab == `Temperature` && 
          <SensorModal logo={ TempLogo } Title={`Temperature`} sensorModel={`SHT31`} currentReading={`Temperature: ${ temperature }`} setTab={ setTab }/>
        }
        { tab == `Carbon Dioxide` && 
          <SensorModal logo={ CarbonLogo } Title={`Carbon Dioxide`} sensorModel={`NDIR CO₂`} currentReading={`Carbon Dioxide: ${ pressure }`} setTab={ setTab }/>
        }
    </>)
} 